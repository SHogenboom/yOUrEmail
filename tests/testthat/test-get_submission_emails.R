...
testthat::test_that("Submissions: Retrieve from Outlook", {
  ...
  testthat::expect_length(
    get_submissions(outlook),
    10
  )

  # TEST: implausible submissions folder
  # Expectation: error due to inability to locate the folder
  testthat::expect_error(get_submissions(outlook,
    submissions_folder = "qwertyuiop"
  ))

  # TEST: implausible submissions email
  # Expectation: success but an empty list of emails
  submission_emails <- get_submissions(outlook,
    submissions_email = "qwertyuiop@ou.nl"
  )
  ...
  testthat::expect_null(get_submissions(outlook,
    n_latest_emails = -10
  ))
})
